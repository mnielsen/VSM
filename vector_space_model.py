"""vector_space_model.py implements a toy search engine to illustrate
...
# We use a corpus of three documents.  It is convenient to introduce
...
# postings: key: term;  value: defaultdict whose keys are document ids, and
# whose values are the frequency with which the term occurs in the document.
postings = defaultdict(defaultdict)
...
def main():
    ...
    # Find the document ids containing all query terms
    ...
    if len(relevant_document_ids) == 0:
        ...
        sys.exit()
    # The following comparison is used to order the scores
    cmp = lambda x,y: x[1] > y[1]
    scores = [(id,similarity(q,id)) 
                    for id in relevant_document_ids].sort(cmp)
...
def init_terms_and_postings():
    ...
    overall dictionary, and adds the document to the posting list for each
    term, together with frequency of the term in the document."""
...
def intersection(sets):
    ...
    error."""
    reduce(set.intersection, [s for s in sets])
